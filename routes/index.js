...
router.get('/search_by_subject',function(req,res){
  if(req.session.data || req.session.passport)
  {
      pool.query('select distinct title,author,subject,publisher,classificationnumber,language,volume from library.books where subject like "%'+req.query.subject+'%" or title like "%'+req.query.subject+'%" or publisher like "%'+req.query.subject+'%" or author like "%'+req.query.subject+'%"',function(error,result){
        ...
        {
          logger.counsellingLogger.log('info',`${req.session.ip1} ${req.session.loguser} searched book with info "${req.query.subject}"`)
...
router.get('/showsearchbook',function(req,res){
  if(req.session.data || req.session.passport)
  {
      ...
      logger.counsellingLogger.log('info',`${req.session.ip1} ${req.session.loguser} rendered search book page`)
      res.render('searchbook',{data:req.session.data})
  }
  else
    res.redirect('/')
})
...
router.get('/showreturnhistory',function(req,res){
  if(req.session.data || req.session.passport)
  { 
    logger.counsellingLogger.log('info',`${req.session.ip1} ${req.session.loguser} rendered return history page`)
      
    res.render('returnhistory',{data:req.session.data,returndata:[]})
  }
  else
    res.redirect('/')
})
...
router.get('/get_issued_books',function(req,res){
  if(req.session.data || req.session.passport)
  {
      pool.query('select * from library.issue where cardnumber=?',[req.query.cardnumber],function(error,result){
        ...
        {
          logger.counsellingLogger.log('info',`${req.session.ip1} ${req.session.loguser} fetched issued books`)
          res.status(200).json(result)
        }
      })
  }
...
router.post('/showuserprofile',function(req,res){
  if(req.session.data || req.session.passport)
  {
      ...
      pool.query('select * from library.userdetails where mailid=?',[req.body.showuserprofile],function(error,result){
        ...
        {
            logger.counsellingLogger.log('error',`${error}`)
            res.render('profile',{msg:'user not found',data:''})
        }
        else
        {
            if(result.length==1)
            {
                ...
                logger.counsellingLogger.log('info',`${req.session.ip1} ${req.session.loguser} opened user profile`)

                res.render('profile',{data:result[0]})
            }
            else
            {
               res.render('profile',{msg:'server error',data:''})
            }
        }
      })
  }
          ...
          try
          {
            ...
            ; // true
              if(result.length==1 && bcrypt.compareSync(req.body.userpassword, result[0].password))
              {
                ...
                logger.counsellingLogger.log('info',`${req.session.ip1} ${req.session.loguser} signed in`)
...
router.get('/userdashboard',function(req,res){
  ...
  {
    req.session.ip1=req.query.googleip
      // logger.counsellingLogger.log('info',`${req.session.ip} ${req.session.data.mailid} ${req.session.passport} signed in`)
      ...
      //   logger.counsellingLogger.log('info',`${req.session.ip1} ${req.session.loguser} render dashboard`)
        ...
        logger.counsellingLogger.log('info',`${req.session.ip1} ${req.session.loguser} rendered dashboard signed in`)

      res.render('userdashboard',{data:req.session.data})
  }
  else
  {
      res.redirect('/')
...
router.post('/adduser',upload.single('profilepicture'),function(req,res){
   ...
   if(req.session.passport)
   {
      ...
      catch(err)
      {
          state='',
          course='',
          req.body.address='',req.body.city='',req.body.branch='',req.body.semester=0
      ...
      pool.query('insert into library.userdetails (mailid,firstname,lastname,address,password,state,city,course,branch,userprofile,dob,semester,role) values(?,?,?,?,?,?,?,?,?,?,?,?,?)',[
          ...
          req.body.firstname,
          req.body.secondname,
          aes256.encrypt(key,req.body.address),
          hashedpass,
          aes256.encrypt(key,state),
          aes256.encrypt(key,req.body.city),
          course,
          req.body.branch,
          req.body.profilepic,
          aes256.encrypt(key,req.body.dob),
          req.body.semester,
          req.body.role
           ...
           pool.query('select * from library.userdetails where mailid=?',[req.user.email],function(error,result){
              ...
              {async function main() {
                ...
                let info = await transporter.sendMail({
                  from: "sdc@mitsgwalior.in", // sender address
                  to: `${req.body.useremailid}`, // list of receivers
                  subject: `Welcome to Library MITS Gwalior`, // Subject line
                ...
                // req.session.addadmin = "admin added";
              ...
              main().catch(
                ...
                // logger.counsellingLogger.log('info', `${req.session.ip} ${req.session.usernameip} dashboard opened `)
                req.session.data=result[0]
                res.redirect('/userdashboard')
              }
           })
      ...
      if(req.user._json.domain=="mitsgwl.ac.in")
        {
           pool.query('select * from library.userdetails where mailid=?',[req.user.email],function(error,result){
              ...
              else
              {
                  if(result.length!=1)
                    {
                      res.render('Resgestration', { data: req.user,states:states,role:'student'});
                    }   
                   else
                   {
                      req.session.data=result[0];
                      req.session.loguser=result[0].firstname
                      // logger.counsellingLogger.log('info',`${req.user.email} signed in through google`)
                      res.render('userdashboard',{data:result[0]});
                   } 
              }
        ...
        else if(req.user._json.domain=="mitsgwalior.in")
        {
          pool.query('select * from library.userdetails where mailid=?',[req.user.email],function(error,result){
            ...
            {
                if(result.length!=1)
                  res.render('Resgestration', { data: req.user,states:states,role:'staff'});   
                 else
                 {
                    req.session.data=result[0];
                    // logger.counsellingLogger.log('info',`${req.user.email} signed in through google`)
...
router.post('/logout', (req, res) => {
  ...
  logger.counsellingLogger.log('info',`${req.session.ip1} ${req.session.loguser} signed out` )
