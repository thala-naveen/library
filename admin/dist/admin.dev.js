  ...
  if (req.session.adminemail) {
    pool.query('delete from library.admin where adminemail=?', [req.query.adminemail], function (error, result) {
      if (error) {
        ...
        logger.counsellingLogger.log('info', "".concat(req.session.ip, " admin ").concat(req.session.adminemail, " deleted admin ").concat(req.query.adminemail));
...
router.post('/add_admin', function (req, res) {
  if (req.session.adminemail) {
    ...
    pool.query('insert into library.admin(adminemail,adminpassword) values(?,?)', [req.body.adminemail, encryptedpass], function (error, result) {
      if (error) {
        if (error.toString().includes('Duplicate')) req.session.adminmsg = "admin already exist";else req.session.adminmsg = "server error";
        res.redirect('/admin/showaddadmin');
      } else {
        req.session.adminmsg = 'admin added';
        logger.counsellingLogger.log('info', "".concat(req.session.ip, " admin ").concat(req.session.adminemail, " added admin ").concat(req.body.adminemail));
        res.redirect('/admin/showaddadmin');
      }
    });
  } else res.redirect('/');
  ...
  if (req.session.adminemail) {
    pool.query('select * from library.admin', function (error, result) {
      if (error) {
        ...
        logger.counsellingLogger.log('info', "".concat(req.session.ip, " admin ").concat(req.session.adminemail, " rendered addadmins page"));
        res.render('addadmins', {
          adminemail: req.session.adminemail,
          msg: req.session.adminmsg,
          admindata: result
        });
      }
    });
  ...
  if (req.session.adminemail) {
    pool.query('delete from library.branch where branchid=?', [req.query.branchid], function (error, result) {
      if (error) {
        ...
        logger.counsellingLogger.log('info', "".concat(req.session.ip, " admin ").concat(req.session.adminemail, " deleted branch of branchid").concat(req.query.branchid));
  ...
  if (req.session.adminemail) {
    ...
    pool.query('insert into library.branch(courseid,branchname,branchcode) values(?,?,?)', [course, req.body.branch, req.body.branchcode], function (error, result) {
      if (error) {
        ...
        logger.counsellingLogger.log('info', "".concat(req.session.ip, " admin ").concat(req.session.adminemail, " added branch ").concat(req.body.branch));
        res.redirect('/admin/showaddbranch');
      }
    });
  } else res.redirect('/');
  ...
  if (req.session.adminemail) {
    pool.query('select * from library.branch', function (error, result) {
      if (error) {
        ...
        logger.counsellingLogger.log('info', "".concat(req.session.ip, " admin ").concat(req.session.adminemail, " rendered showaddbranch page"));
        res.render('addbranch', {
          msg: req.session.msg,
          branchdata: result,
          adminemail: req.session.adminemail
  ...
  if (req.session.adminemail) {
    pool.query('delete from library.course where courseid=?', [req.query.courseid], function (error, result) {
      if (error) {
        ...
        logger.counsellingLogger.log('info', "".concat(req.session.ip, " admin ").concat(req.session.adminemail, " deleted course of courseid ").concat(req.query.courseid));
        res.redirect('/admin/showaddprogram');
      }
    });
  } else res.redirect('/');
  ...
  if (req.session.adminemail) {
    pool.query('insert into library.course(coursename) values(?)', [req.body.program.trim()], function (error, result) {
      if (error) {
        ...
        logger.counsellingLogger.log('info', "".concat(req.session.ip, " admin ").concat(req.session.adminemail, " added program/course ").concat(req.body.program));
        res.redirect('/admin/showaddprogram');
      }
    });
  } else res.redirect('/');
  ...
  if (req.session.adminemail) {
    pool.query('select * from library.course', function (error, result) {
      if (error) {
        ...
        logger.counsellingLogger.log('info', "".concat(req.session.ip, " admin ").concat(req.session.adminemail, " rendered showaddprogram"));
        res.render('addprogram', {
          programdata: result,
          msg: req.session.msg,
          adminemail: req.session.adminemail
  ...
  if (req.session.adminemail) {
    pool.query('update library.files set filename=? where fileid=3', [req.file.originalname], function (error, result) {
      if (error) {
        ...
        var UploadCsvDataToMySQL = function UploadCsvDataToMySQL(filePath) {
          ...
          var csvStream = csv.parse().on("data", function (data) {
            ...
            pool.query("insert into library.books(barcode,author,title,volume,publisher,copyright,pages,classificationnumber,language,stockhead,issueinglibrary,subject,price,currency,billdate,billnumber,bookcategory,section,booktype,sourceofpurchase,loststatus) values ?", [csvData], function (error, result) {
              if (error) {
                ...
                logger.counsellingLogger.log('info', "".concat(req.session.ip, " admin ").concat(req.session.adminemail, " added books from ").concat(req.file.originalname));
                res.redirect('/admin/showaddbooks');
              }
            });
          });
  ...
  if (req.session.adminemail) {
    pool.query('select * from library.books', function (error, result) {
      if (error) {
        ...
        logger.counsellingLogger.log('info', "".concat(req.session.ip, " admin ").concat(req.session.adminemail, " rendered show addbooks page"));
        res.render('addbooks', {
          adminemail: req.session.adminemail,
          msg: req.session.issuemsg,
          data: result
        });
      }
    });
    ...
    pool.query('select * from library.userdetails where firstname=?', [req.query.studenten], function (error, result) {
      if (error) {
        ...
        if (result.length == 1) {
          ...
          logger.counsellingLogger.log('info', "".concat(req.session.ip, " admin ").concat(req.session.adminemail, " searched student ").concat(result[0].firstname));
  ...
  if (req.session.adminemail) {
    pool.query('update library.userdetails set remark=? where firstname=?', [req.body.remark, req.body.studentcard], function (error, result) {
      if (error) {
              ...
              switch (_context.prev = _context.next) {
                ...
                case 2:
                  ...
                  transporter = nodemailer.createTransport({
                    ...
                    port: 465,
                    auth: {
                      user: 'librarymitramits@gmail.com',
                      pass: 'vlqwlelvtklqiugu'
                    }
                  });
                  _context.next = 6;
                  return regeneratorRuntime.awrap(transporter.sendMail({
                    from: 'librarymitramits@gmail.com',
                    // sender address
                    to: "".concat(req.session.usermail),
                    // list of receivers
                    subject: "Library Remark",
                    // Subject line
                    text: "".concat(req.body.remark) // plain text body
                ...
                case 6:
                  ...
                  logger.counsellingLogger.log('info', "".concat(req.session.ip, " admin ").concat(req.session.adminemail, " remarked ").concat(req.body.studentcard, ":").concat(req.body.remark)); // Preview only available when sending through an Ethereal account
        ...
        main()["catch"](logger.counsellingLogger.log('error', "".concat(console.error)));
        logger.counsellingLogger.log('info', "".concat(req.session.ip, " admin ").concat(req.session.adminemail, " added remark ").concat(req.body.remark, " for user ").concat(req.body.studentcard));
        res.redirect('/admin/showaddremark');
      }
    });
  } else res.redirect('/');
  ...
  if (req.session.adminemail) {
    pool.query('update library.userdetails set remark=NULL where firstname=?', [req.query.en], function (error, result) {
      if (error) {
        ...
        logger.counsellingLogger.log('info', "".concat(req.session.ip, " admin ").concat(req.session.adminemail, " removed remark for user ").concat(req.query.en));
        res.redirect('/admin/showaddremark');
      }
    });
  } else res.redirect('/');
  ...
  if (req.session.adminemail) {
    pool.query('select * from library.userdetails where remark is not null', function (error, result) {
      if (error) {
        ...
        req.session.remark;
        logger.counsellingLogger.log('info', "".concat(req.session.ip, " admin ").concat(req.session.adminemail, " rendered add remark page"));
        res.render('addremark', {
          adminemail: req.session.adminemail,
          remarkdata: result,
          msg: req.session.searchmsg,
          studenten: req.session.studenten,
  ...
  if (req.session.adminemail) {
    pool.query('delete from library.notice where noticeid=?', [req.query.noticeid], function (error, result) {
      if (error) {
        ...
        logger.counsellingLogger.log('info', "".concat(req.session.ip, " admin ").concat(req.session.adminemail, " removed notice of noticeid ").concat(req.query.noticeid));
        res.redirect('/admin/showaddnotice');
      }
    });
  } else res.redirect('/');
    ...
    pool.query('insert into library.notice(notice) values(?)', [req.body.notice], function (error, result) {
      if (error) {
        ...
        res.render('addnotice', {
          msg: 'server error',
          adminemail: req.session.adminemail,
          noticedata: req.session.noticedata
        });
      } else {
        logger.counsellingLogger.log('info', "".concat(req.session.ip, " admin ").concat(req.session.adminemail, " added notice ").concat(req.body.notice));
        res.redirect('/admin/showaddnotice');
      }
    });
  } else res.redirect('/');
  ...
  if (req.session.adminemail) {
    pool.query('select * from library.notice', function (error, result) {
      if (error) {
        ...
        logger.counsellingLogger.log('info', "".concat(req.session.ip, " admin ").concat(req.session.adminemail, " rendered show notice page"));
        res.render('addnotice', {
          adminemail: req.session.adminemail,
          msg: '',
          noticedata: result
    ...
    if (req.body.issue == "upload") {
      pool.query('update library.files set filename=? where fileid=1', [req.file.originalname], function (error, result) {
        if (error) {
            ...
            var csvStream = csv.parse().on("data", function (data) {
              ...
              pool.query("insert into library.issue(issuedate,barcode,title,author,cardnumber,surname) values ?", [csvData], function (error, result) {
                if (error) {
                  ...
                  logger.counsellingLogger.log('info', "".concat(req.session.ip, " admin ").concat(req.session.adminemail, " added issue file"));
                  req.session.issuemsg = "issue file uploaded";
                  res.redirect('/admin/showaddfiles'); // res.render('addfiles',{issuemsg:'issue file uploaded',adminemail:req.session.adminemail,returnmsg:'',duplicateIssueError:req.session.duplicateIssueError})
                }
              });
            });
            stream.pipe(csvStream);
          ...
          req.session.filename = req.file.originalname;
          UploadCsvDataToMySQL("public/files/".concat(req.file.originalname));
        }
      });
      ...
      pool.query('select filename from library.files where fileid=1', function (error, result) {
        if (error) {
          logger.counsellingLogger.log('error', "".concat(error));
          res.render('addfiles', {
            issuemsg: 'server error',
            adminemail: req.session.adminemail,
            returnmsg: ''
          });
        } else {
          try {
            fs.unlinkSync("public/files/".concat(result[0].filename));
            pool.query('truncate library.issue', function (error, result) {
              if (error) {
                ...
                logger.counsellingLogger.log('info', "".concat(req.session.ip, " admin ").concat(req.session.adminemail, " removed issue file"));
                res.render('addfiles', {
                  issuemsg: 'file removed',
                  adminemail: req.session.adminemail,
                  returnmsg: ''
                });
              }
                ...
                if (error) {
                  ...
                  pool.query('delete from library.issue where barcode in (SELECT barcode FROM library.return) and cardnumber in (select cardnumber from library.return)', function (error, result) {
                    if (error) {
                      ...
                      logger.counsellingLogger.log('info', "".concat(req.session.ip, " admin ").concat(req.session.adminemail, " added return file"));
              ...
              if (error) {
                ...
                pool.query('truncate library.return', function (error, result) {
                  if (error) {
                    ...
                    logger.counsellingLogger.log('info', "".concat(req.session.ip, " admin ").concat(req.session.adminemail, " removed return file"));
...
router.get('/showaddfiles', function (req, res) {
  if (req.session.adminemail) {
    var returnmsg = ''; // let returnmsg =  req.session.returnmsg=="return file uploaded"?"return file uploaded":""

    var issuemsg = req.session.issuemsg == "issue file uploaded" ? "issue file uploaded" : "";
  ...
  if (req.session.adminemail) {
    pool.query('select * from library.userdetails', function (error, result) {
      if (error) {
        ...
        logger.counsellingLogger.log('info', "".concat(req.session.ip, " admin ").concat(req.session.adminemail, " rendered show users page"));
    ...
    if (error) {
      ...
      try {
        ...
        if (result.length == 1 && decryptPassword == req.body.adminpassword) {
          ...
          logger.counsellingLogger.log('info', "".concat(req.session.ip, " admin ").concat(req.session.adminemail, " logged in"));
...
router.get('/admindashboard', function (req, res) {
  if (req.session.adminemail) {
    logger.counsellingLogger.log('info', "".concat(req.session.ip, " admin ").concat(req.session.adminemail, " rendered dashboard"));
...
router.get('/adminlogout', function (req, res) {
  logger.counsellingLogger.log('info', "".concat(req.session.ip, " admin ").concat(req.session.adminemail, " signed out"));
  req.session.destroy();
  res.redirect('/');
});
router.get('/adminlogin', function (req, res) {
  res.render('adminlogin', {
    msg: '',
    invailderror: req.session.invaliderror
  });
});
